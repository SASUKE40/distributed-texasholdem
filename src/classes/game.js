...
const Game = function (name, host) {
  ...
  this.assignBlind = () => {
    this.roundData.smallBlind =
      this.roundData.dealer + 1 < this.players.length ? this.roundData.dealer + 1 : 0;
    this.roundData.bigBlind =
      this.roundData.smallBlind + 1 < this.players.length ? this.roundData.smallBlind + 1 : 0;
    ...
    const goFirstIndex =
      this.roundData.bigBlind - 1 < 0 ? this.players.length - 1 : this.roundData.bigBlind - 1;
    ...
    if (this.roundData.bets == undefined || this.roundData.bets.length == 0) return 0;
      ...
      for (let i = 0; i < this.roundData.bets.length; i++) {
        sum += this.roundData.bets[i].reduce(
          (acc, curr) => (curr.bet != "Buy-in" && curr.bet != "Fold" ? acc + curr.bet : acc + 0),
          0,
        );
      }
  ...
  this.isStageComplete = () => {
    ...
    if (this.roundData.bets.length == 1) {
      allPlayersPresent =
        currRound.filter((a) => a.bet != "Fold").length >= numUnfolded && this.bigBlindWent;
    } else {
    ...
    if (this.getCurrentRoundBets().some((a) => a.player == player.getUsername())) {
      this.setCurrentRoundBets(
        this.getCurrentRoundBets().map((a) =>
          a.player == player.getUsername() ? { player: player.getUsername(), bet: "Fold" } : a,
        ),
      );
    } else {
    ...
    if (currBet === 0) {
      if (this.getCurrentRoundBets().some((a) => a.player == player.getUsername())) {
        if (player.getMoney() - topBet <= 0) {
          ...
          this.setCurrentRoundBets(
            this.getCurrentRoundBets().map((a) =>
              a.player == player.username ? { player: player.getUsername(), bet: topBet } : a,
            ),
          );
  ...
  this.bet = (socket, bet) => {
    ...
    if (bet >= this.bigBlind) {
      ...
      if (player.getMoney() - bet >= 0) {
        this.setCurrentRoundBets(
          this.getCurrentRoundBets().filter((a) => a.player != player.getUsername()),
        );
        this.getCurrentRoundBets().push({
          player: player.getUsername(),
          bet: bet,
  ...
  this.check = (socket) => {
    ...
    if (this.getCurrentRoundBets().find((a) => a.player == player.getUsername()) != undefined) {
      ...
      this.setCurrentRoundBets(
        this.getCurrentRoundBets().map((a) =>
          a.player == player.getUsername() ? { player: player.getUsername(), bet: currBet } : a,
        ),
      );
    } else {
      this.getCurrentRoundBets().push({
  ...
  this.raise = (socket, bet) => {
    ...
    if (moneyToRemove > 0 && bet >= topBet && player.getMoney() - moneyToRemove >= 0) {
      if (currBet === 0) {
        ...
        this.setCurrentRoundBets(
          this.getCurrentRoundBets().map((a) =>
            a.player == player.getUsername() ? { player: player.getUsername(), bet: bet } : a,
          ),
        );
      }
  ...
  this.getPossibleMoves = (socket) => {
    ...
    if (topBet != 0) {
      ...
      if (player.blindValue == "Big Blind" && !this.bigBlindWent && topBet == this.bigBlind)
